...
class ContactsImporter {
	...
	public function import() {
		...
		while(!$end && $cc<10) {
			...
			$curl_handle = curl_init();
			$url = "https://api.leadchampion.com/v1/contacts?datefrom=".urlencode($date_from)."&dateto=".urlencode($date_to)."&limit=250&offset=".$offset;
			...
			error_log("Richiamo nr.".($cc+1)." : ".count($response_data2["contacts"]));
			
			$response_data = array_merge($response_data, $response_data2);
		...
		error_log("Total records : ".count($response_data["contacts"]));
