...
AND DRRT='RY'  
";
if($SHRYIN!="") $query .= " AND TRIM(DRKY) = ?  ";
$pstmt = odbc_prepare($conn,$query);

$r = 0; 
if($pstmt) {
	$arrParams = array();		
	if($SHRYIN!="") $arrParams[] = trim($SHRYIN);			 
	...
	if($res) {
		while($row = odbc_fetch_array($pstmt)) {
			...
			AND TRIM(DRKY) = ?   
			";
			if($DRLNGP!="") $query.=" AND TRIM(DRLNGP) = ? ";
			...
			$dspVal = true;
			if($filter_desc!="") {
				if(strpos(strtolower($DRDL01), strtolower($filter_desc)) === false) $dspVal = false;
			}
			
			if($dspVal) {
				if($r>0) echo ',';
				echo '{"payment_method_lang":'.json_encode($DRLNGP).',"payment_method_code":'.json_encode($DRKY).',"payment_method_descr":'.json_encode($DRDL01).'}';
				$r++;
			}
