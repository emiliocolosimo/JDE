...
class WebSmartObject
{
	...
	function __construct()
	{
		...
		if (isset($_SERVER['REQUEST_METHOD']))
		{
			...
			{
				case 'GET': $this->request_data = &$_GET; 
							break;
				case 'POST': $this->request_data = &$_POST; 
							break;
				case 'HEAD': $this->request_data = &$_HEAD; 
							break;
				case 'PUT': $this->request_data = &$_PUT; 
							break;
				default: $this->request_data = &$_POST; 	
		    }
		}
		
		if(isset($this->request_data['task']))
		{
			$this->pf_task = $this->request_data['task'];
		}
		else
		{
	...
	private function setDBOptions()
	{
		...
		if(strlen($this->defaults['pf_db2LibraryList']) == 0 && count($this->pf_liblLibs) > 0 && is_array($this->pf_liblLibs )) 
		{
			...
			$this->defaults['pf_db2OdbcLibraryList'] = ';DBQ=,' . implode(' ', $this->pf_liblLibs);
		}
	...
	private function getParm(&$field, $key, $enctype = '')
	{
		$field = xl_get_parameter($key, $enctype);
	}
	...
	{
		// set the color
		if(!isset($this->pf_altrowclr) || $this->pf_altrowclr != $color1)
			$this->pf_altrowclr = $color1;
		else
			$this->pf_altrowclr = $color2;
	}
	...
	// Retrieve the stored program state
	function getState()
	{
		// Retrieve the stored program state, and merge it with the defaults
		if (isset($_SESSION[$this->pf_scriptname]))
		{
			// Merge filters from programState and Session into Session, as it will take priority in the next merge
			if (isset($this->programState['filters']) && isset($_SESSION[$this->pf_scriptname]['filters']))
			{
				$_SESSION[$this->pf_scriptname]['filters'] = array_merge($this->programState['filters'], 
					$_SESSION[$this->pf_scriptname]['filters']);
			}
			$this->programState = array_merge($this->programState, $_SESSION[$this->pf_scriptname]);
		}
	}
	...
	protected function displayError($fieldname, $replacements = array())
	{
		if(!array_key_exists($fieldname, $this->formFields)) 
		{
			return;
		}
		if(array_key_exists('errorText', $this->formFields[$fieldname]))
		{
			vprintf($this->formFields[$fieldname]['errorText'], $replacements);
		}
	}
	
	protected function validate($protectedFields = array())
	{
	...
	protected function displayIndicator($fieldname)
	{		
		if($this->showRequiredIndicator)
		{
			$this->displayRequiredIndicator($fieldname);
			
		}
		
		if($this->showOptionalIndicator)
		{
			$this->displayOptionalIndicator($fieldname);
		}
	}
	
	protected function displayRequiredIndicator($fieldname)
	{		
		//make sure the field is part of the formfields array
		if(!array_key_exists($fieldname, $this->formFields)) 
		{
			return;
		}
		
		//have to check if we have validators before we can check if the field is an
		//optional or required field
		if(!array_key_exists('validators', $this->formFields[$fieldname]))
		{
			return;
		}
		
		if(in_array($this->requiredValidatorName, $this->formFields[$fieldname]['validators']) || 
			array_key_exists($this->requiredValidatorName, $this->formFields[$fieldname]['validators']))
		{
			echo "<span class=\"required\">" . $this->requiredIndicator . "</span>";
		}
	}
	
	protected function displayOptionalIndicator($fieldname)
	{
		if(!array_key_exists($fieldname, $this->formFields))
		{
			echo "<span class=\"optional\">" . $this->optionalIndicator . "</span>";
		}
		
		if(array_key_exists($fieldname, $this->formFields) && 
			(array_key_exists('validators', $this->formFields[$fieldname]) && !in_array($this->requiredValidatorName, $this->formFields[$fieldname]['validators'])
			&& !array_key_exists($this->requiredValidatorName, $this->formFields[$fieldname]['validators'])))
		{
			echo "<span class=\"optional\">" . $this->optionalIndicator . "</span>";
		}
	...
	protected function displayErrorClass($fieldname)
	{
		if(!array_key_exists($fieldname, $this->formFields)) 
		{
			return;
		}
		if(array_key_exists('hasError', $this->formFields[$fieldname]))
		{
			echo $this->hasErrorClass;
		}
	}
