...
class WebSmartObject
{
	...
	protected function checkRequestMethod(...$methods)
	{
		...
		if (!$isAllowed)
		{
			$this->closeConnectionWithError('error', 'Request method not allowed for this task.');
		}
	}
	...
	protected function closeConnectionWithError($status, $message)
	{
		db2_close($this->db_connection);
		$this->setMessageWithStatus($status, $message);
		die(json_encode($this->jsonOutput));
	}

	protected function setMessageWithStatus($status, $message)
	{
