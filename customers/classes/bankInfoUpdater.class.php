...
class BankInfoUpdater {
	...
	public function recordExists() {
		...
		$query = "SELECT 'S' AS RECEX FROM ".$this->curLib.".F0030 WHERE 
		";
		...
		if(!$pstmt) {
			$hasErrors = true;
			$arrErrors[$xe]["field"] = "";
			$arrErrors[$xe]["msg"] = "F01017:".odbc_errormsg($this->odbc_conn);
			$xe++;			
			return array("hasErrors"=>$hasErrors,"arrErrors"=>$arrErrors);
		}		
		for($i=0;$i<count($keyFields);$i++) {
		...
		$row = odbc_fetch_array($pstmt);
		if($row && isset($row["RECEX"]) && $row["RECEX"]=="S") return true;
		return false;
	...
	public function execUpdate() {
		...
		$query = "UPDATE ".$this->curLib.".F0030 SET "; 
		$xi = 0;
		for($i=0;$i<count($inputFields);$i++) { 
