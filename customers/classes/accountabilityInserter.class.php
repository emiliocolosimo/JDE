...
class AccountabilityInserter {
	...
	public function checkMandatoryFields($fieldsArray) {
		...
		if($headerFields["customer_type"]=="C") $fieldsArray["A5AN8"] = $fieldsArray["A5AN8"] + 1000000;
		if($headerFields["customer_type"]=="P") $fieldsArray["A5BADT"] = "X";
		if($headerFields["customer_type"]=="PS") $fieldsArray["A5BADT"] = "S";
	...
	public function execInsert() {
		...
		$query = "INSERT INTO ".$this->curLib.".F0301 (".implode(",",$inputFields).") VALUES("; 
		for($i=0;$i<count($inputFields);$i++) { 
			if($i>0) $query.= ",";
