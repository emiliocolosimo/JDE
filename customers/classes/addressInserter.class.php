...
class AddressInserter {
	...
	public function setF0111Fields($F0111Fields) {

		$this->F0111Fields = $F0111Fields;
		return true;
	}	
	...
	public function checkMandatoryFields($fieldsArray) {
		...
		$fieldsArray["ALAN8"] = $headerFields["customer_code"];
		if($headerFields["customer_type"]=="C") $fieldsArray["ALAN8"] = $fieldsArray["ALAN8"] + 1000000;
		 
		$fieldsArray["ALUSER"] = $this->getUtenteJDE($headerFields["crm_user"]);
		$fieldsArray["ALPID"] = $fieldsArray["ALUSER"];
		$fieldsArray["ALTORG"] = $fieldsArray["ALUSER"];		
		
		//$F0111Fields 
		for($i=0;$i<count($F0111Fields);$i++) { 
			$currentResArray = $F0111Fields[$i];  
			if($currentResArray["customer_contact_type"]=="" && strlen($currentResArray["contact_description"]) > 40) {
				$fieldsArray["ALADD1"] = substr($currentResArray["contact_description"],40);
			}
		}
		//..
	...
	public function execInsert() {
		...
		$query = "INSERT INTO ".$this->curLib.".F0116 (".implode(",",$inputFields).") VALUES("; 
		for($i=0;$i<count($inputFields);$i++) { 
			if($i>0) $query.= ",";
		...
		if(!$pstmt) {
			$hasErrors = true;
			$arrErrors[$xe]["field"] = "";
			$arrErrors[$xe]["msg"] = "F0116:".odbc_errormsg($this->odbc_conn);
			$xe++;			
			return array("hasErrors"=>$hasErrors,"arrErrors"=>$arrErrors);
		}
		$arrParams = array();	
