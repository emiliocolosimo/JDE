...
class CustomerInserter {
	...
	public function checkMandatoryFields($fieldsArray) {
		...
		if($fieldsArray["ABAT1"]=="C") $fieldsArray["ABAN8"] = $fieldsArray["ABAN8"] + 1000000;
		$fieldsArray["ABDC"] = substr(str_replace(array(" "),array(""),$fieldsArray["ABALPH"]),0,40); 
		$fieldsArray["ABPA8"] = $fieldsArray["ABAN8"];
		$fieldsArray["ABAN81"] = $fieldsArray["ABAN8"];
		$fieldsArray["ABAN82"] = $fieldsArray["ABAN8"];
		$fieldsArray["ABAN83"] = $fieldsArray["ABAN8"];
		$fieldsArray["ABAN84"] = $fieldsArray["ABAN8"];
		$fieldsArray["ABAN85"] = $fieldsArray["ABAN8"];
		$fieldsArray["ABAN86"] = $fieldsArray["ABAN8"];
		 
		$fieldsArray["ABUSER"] = $this->getUtenteJDE($fieldsArray["ABUSER"]);
		$fieldsArray["ABPID"] = $fieldsArray["ABUSER"];
		$fieldsArray["ABTORG"] = $fieldsArray["ABUSER"];		
	...
	public function execInsert() {
		...
		$query = "INSERT INTO ".$this->curLib.".F0101 (".implode(",",$inputFields).") VALUES("; 
		for($i=0;$i<count($inputFields);$i++) { 
			if($i>0) $query.= ",";
