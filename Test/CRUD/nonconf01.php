...
//	Program Title:		Non conformit&agrave;  - elenco ordini
//	Created by:			matti
...
class nonconf01 extends WebSmartObject
{
	protected $programState = array(
		...
		'page' => 1,
		'listSize' => 20,
		'filters' => array('tipo' => '', 'SDAN8' => '', 'ABALPH' => '', 'SDDOCO' => '', 'SDLOTN' => '', 'SDLITM' => '')
	);
	...
	protected function cvtDateUSToEU($date) { 
	...
	protected function deleteRecord() 
	{
		...
		// Prepare and execute the SQL statement to delete the record
		$delString = 'DELETE FROM JRGDTA94C.F4211 ' . $this->buildRecordWhere() . ' WITH NC';
	...
	protected function endAdd()
	{
		...
		$insertSql = 'INSERT INTO JRGDTA94C.F4211 (SDDCTO, SDLNID, SDPSN, SDDELN, SDDOC, SDLITM, SDDSC1, SDLOTN, SDIVD) VALUES(:SDDCTO, :SDLNID, :SDPSN, :SDDELN, :SDDOC, :SDLITM, :SDDSC1, :SDLOTN, :SDIVD)' . ' WITH NC';
	...
	protected function endChange()
	{
		...
		$updateSql = 'UPDATE JRGDTA94C.F4211 SET SDDCTO = :SDDCTO, SDLNID = :SDLNID, SDPSN = :SDPSN, SDDELN = :SDDELN, SDDOC = :SDDOC, SDLITM = :SDLITM, SDDSC1 = :SDDSC1, SDLOTN = :SDLOTN, SDIVD = :SDIVD';
	...
	protected function filterList()
	{
		...
		$this->programState['filters']['SDLITM'] = xl_get_parameter('filter_SDLITM');
	...
	protected function buildListCliente() {
		...
		while ($row && ($rowCount < $listSize))
		{
			...
			//cerco tra non conformità ordini associati:
			if($HASNONCONF!="S") {
				$selString = "SELECT 'S' AS HASNONCONF 
				FROM BCD_DATIV2.NONCOL0F 
				JOIN BCD_DATIV2.NONCON0F ON OLPRNC = NCPROG 
				WHERE NCTPCF = 'C' 
				AND OLDCTO = :NCDCTO 
				AND OLDOCO = :NCDOCO 
				AND OLLNID = :NCLNID 
			...
			//cerco tra non conformità ordini associati [f]
			
			
			$SDLNID = (int) $SDLNID / 100;
	...
	protected function buildListFornitore() {
		...
		while ($row && ($rowCount < $listSize))
		{
			...
			// Urlencode the key fields so we can use that form on the HTML output
			$PDKCOO_url = urlencode(rtrim($row['PDKCOO']));
			$PDDOCO_url = urlencode(rtrim($row['PDDOCO']));
			...
			$stmtNC->bindValue(':NCLOTN', $PRLOTN, PDO::PARAM_STR);
	...
	protected function buildListStmtCliente($rowOffset, $listSize)
	{
		...
		{
			$stmt->bindValue(':SDAN8', strtolower($this->programState['filters']['SDAN8']), PDO::PARAM_INT);
		}
		...
		{
			$stmt->bindValue(':SDDOCO', strtolower($this->programState['filters']['SDDOCO']), PDO::PARAM_INT);
		}
		...
		if ($this->programState['filters']['SDLITM'] != '')
		{
			$stmt->bindValue(':SDLITM', '%' . strtolower($this->programState['filters']['SDLITM']) . '%', PDO::PARAM_STR);
		}
	...
	protected function buildListStmtFornitore($rowOffset, $listSize)
	{
		...
		JRGDTA94C.F43121.PRLOTN, CAST(JRGDTA94C.F4311.PDUORG / 100 AS INTEGER) ';
	...
	{
		$selString = '
		SELECT T.RRNF, T.SDAN8, TRIM(JRGDTA94C.F0101.ABALPH) AS ABALPH, T.SDKCOO, 
		T.SDDOCO, T.SDDCTO, T.SDLNID, 
		T.SDPSN, T.SDDELN, T.SDDOC, 
		TRIM(T.SDLITM) AS SDLITM, T.SDDSC1, TRIM(T.SDLOTN) AS SDLOTN,  
		T.SDIVD, T.TIPOFILE, T.SDLNTY, CAST(T.SDUORG / 100 AS INTEGER) AS SDUORG     
		FROM TABLE ( 
			SELECT RRN(JRGDTA94C.F4211) AS RRNF, JRGDTA94C.F4211.SDAN8, JRGDTA94C.F4211.SDKCOO, 
			...
			WHERE SDDCTO NOT IN (\'ST\' , \'OF\' , \'SQ\') AND SDLNTY=\'S\'
			
			UNION 
			
			SELECT RRN(JRGDTA94C.F42119) AS RRNF, JRGDTA94C.F42119.SDAN8, JRGDTA94C.F42119.SDKCOO, 
		...
		inner join JRGDTA94C.F0101 on T.SDAN8 = JRGDTA94C.F0101.ABAN8		
	...
	protected function buildWhereClauseCliente()
	{
		...
		if ($this->programState['filters']['SDAN8'] != '')
		{
			$whereClause = $whereClause . $link . ' lower(T.SDAN8) = :SDAN8';
			$link = ' AND ';
		}
		...
		if ($this->programState['filters']['SDDOCO'] != '')
		{
			$whereClause = $whereClause . $link . ' lower(T.SDDOCO) = :SDDOCO';
			$link = " AND ";
		}
		...
		if ($this->programState['filters']['SDLITM'] != '')
		{
			$whereClause = $whereClause . $link . ' lower(T.SDLITM) LIKE :SDLITM';
			$link = " AND ";
		}		
	...
	protected function buildWhereClauseFornitore()
	{
		...
		if ($this->programState['filters']['SDAN8'] != '')
		{
			$whereClause = $whereClause . $link . ' lower(JRGDTA94C.F4311.PDAN8) = :SDAN8';
			$link = ' AND ';
		}
		...
		if ($this->programState['filters']['SDDOCO'] != '')
		{
			$whereClause = $whereClause . $link . ' lower(JRGDTA94C.F4311.PDDOCO) = :SDDOCO';
			$link = " AND ";
		}
		...
		if ($this->programState['filters']['SDLOTN'] != '')
		{
			$whereClause = $whereClause . $link . ' lower(JRGDTA94C.F43121.PRLOTN) LIKE :SDLOTN';
			$link = " AND ";
		}
		...
		if ($this->programState['filters']['SDLITM'] != '')
		{
			$whereClause = $whereClause . $link . ' lower(JRGDTA94C.F4311.PDLITM) LIKE :SDLITM';
			$link = " AND ";
		}
	...
	{
		$selString = 'SELECT JRGDTA94C.F4211.SDKCOO, JRGDTA94C.F4211.SDDOCO, JRGDTA94C.F4211.SDDCTO, JRGDTA94C.F4211.SDLNID, JRGDTA94C.F4211.SDPSN, JRGDTA94C.F4211.SDDELN, JRGDTA94C.F4211.SDDOC, JRGDTA94C.F4211.SDLITM, JRGDTA94C.F4211.SDDSC1, JRGDTA94C.F4211.SDLOTN, JRGDTA94C.F4211.SDIVD FROM JRGDTA94C.F4211 inner join JRGDTA94C.F0101 on JRGDTA94C.F4211.SDAN8 = JRGDTA94C.F0101.ABAN8';
	...
	{
		$whereClause = 'WHERE JRGDTA94C.F4211.SDKCOO = :SDKCOO_ AND JRGDTA94C.F4211.SDDOCO = :SDDOCO_';
	...
	{
		$selString = 'SELECT COUNT(*) FROM JRGDTA94C.F4211';
...
<html>
  <head>
    ...
    <title>Non conformit&agrave; - elenco ordini</title>
    ...
    <div id="outer-content">
      ...
      <div id="page-content">
        <div id="content-header">
          <h1 class="title">Non conformit&agrave; - elenco ordini</h1> 
          <span class="add-link">
            <a class="btn btn-info btn-sm" href="nonconf02.php">
              <span class="glyphicon glyphicon-list"></span> Vai a Elenco non conformità
            </a>
          </span> 
        </div>
...
>Fornitore</option>
                ...
                <div class="filter-group form-group col-sm-4 col-lg-2">
                  <label for="filter_SDLITM">Cod. Articolo</label>
                  <input id="filter_SDLITM" class="form-control" type="text" name="filter_SDLITM" maxlength="25" value="{$programState['filters']['SDLITM']}"/>
                </div>
              </div>
              <div class="row">
                <div class="col-sm-2">
...
<html>
  <head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta http-equiv="Pragma" content="no-cache" />
    <title>Non conformit&agrave; - elenco ordini - $mode</title>
  ...
  <body class="single-record display-record">
    <div id="outer-content">
      ...
      <div id="page-content">
        <div id="content-header">
          <h1 class="title">Non conformit&agrave; - elenco ordini - $mode</h1>
        </div>
        <div class="clearfix"></div>
        <div id="contents">
...
<html>
  <head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta http-equiv="Pragma" content="no-cache" />
    <title>Non conformit&agrave; - elenco ordini - Add</title>
  ...
  <body class="single-record manage-record">
    <div id="outer-content">
      ...
      <div id="page-content">
        <div id="content-header">
          <h1 class="title">Non conformit&agrave; - elenco ordini - Add</h1>
        </div>
        <div class="clearfix"></div>
        <div id="contents"> 
...
<html>
  <head>
    ...
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta http-equiv="Pragma" content="no-cache" />
    <title>Non conformit&agrave; - elenco ordini - Change</title>
  ...
  <body class="single-record manage-record">
    <div id="outer-content">
      ...
      <div id="page-content">
        <div id="content-header">
          <h1 class="title">Non conformit&agrave; - elenco ordini - Change</h1>
        </div>
        <div class="clearfix"></div>
        <div id="contents"> 
	...
	if($xlSegmentToWrite == "tabheadercliente")
	{
		...
		echo <<<SEGDTA
          <span id="list-paging-top" class="list-paging">
            <a id="previous-link-top" class="list-paging previous hidden" href="#" onclick="$.blockUI();">
              <span class="glyphicon glyphicon-triangle-left" style="text-decoration: none; font-size: 0.8em"></span> <span>Previous</span>
            </a>
            <a id="next-link-top" class="list-paging next hidden" href="#" onclick="$.blockUI();">
          ...
          <table id="list-table" class="main-list table table-striped" cellspacing="0">
            <thead>
              <tr class="list-header">
                ...
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidx=SDAN8&amp;rnd=$rnd">Cod.Cliente</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidx=ABALPH&amp;rnd=$rnd">Rag.Soc.</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidx=SDDCTO&amp;rnd=$rnd">Tipo ordine</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidx=SDDOCO&amp;rnd=$rnd">Ordine</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidx=SDLNID&amp;rnd=$rnd">Linea</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidx=SDPSN&amp;rnd=$rnd">Prebolla</a>
                </th>
                <th>
                  <a class="list-header" href="$pf_scriptname?sidx=SDDELN&amp;rnd=$rnd">Bolla</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidx=SDDOC&amp;rnd=$rnd">Fattura</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidx=SDLITM&amp;rnd=$rnd">Articolo</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidx=SDDSC1&amp;rnd=$rnd">Descrizione articolo</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidx=SDLOTN&amp;rnd=$rnd">Lotto</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidx=SDUORG&amp;rnd=$rnd">Qta</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidx=SDIVD&amp;rnd=$rnd">Data fattura</a>
                </th>
              </tr>
            </thead>
            <tbody>
...
SEGDTA;
 echo number_format($SDUORG,0,"","."); 
  ...
  <td class="text">
SEGDTA;
 echo $this->cvtDateUSToEU($SDIVD); 
		echo <<<SEGDTA
</td>
</tr>
<tr id="row-expand-cli-$TIPOFILE-$RRNF" style="display:none;">
	<td id="cell-expand-cli-$TIPOFILE-$RRNF" colspan="14"></td>
</tr>
SEGDTA;
		return;
	}
          ...
          <table id="list-table" class="main-list table table-striped" cellspacing="0">
            <thead>
              <tr class="list-header">
                <th class="actions" width="100">Action</th>  
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidxf=PDAN8&amp;rnd=$rnd">Cod.Cliente</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidxf=ABALPH&amp;rnd=$rnd">Rag.Soc.</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidxf=PDDCTO&amp;rnd=$rnd">Tipo ordine</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidxf=PDDOCO&amp;rnd=$rnd">Ordine</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidxf=PDLNID&amp;rnd=$rnd">Linea</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" onclick="$.blockUI();" href="$pf_scriptname?sidxf=PRVRMK&amp;rnd=$rnd">Fattura</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidxf=PDLITM&amp;rnd=$rnd">Articolo</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidxf=PDDSC1&amp;rnd=$rnd">Descrizione articolo</a>
                </th>
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidxf=PRLOTN&amp;rnd=$rnd">Lotto</a>
                </th> 
                <th>
                  <a class="list-header" onclick="$.blockUI();" href="$pf_scriptname?sidxf=PDUORG&amp;rnd=$rnd">Qta</a>
                </th> 
              </tr>
            </thead>
            <tbody>
...
" class="btn btn-default btn-xs glyphicon glyphicon-arrow-right" id="btn-expand-for-$RRNF" title="Visualizza non conformità" href="javascript:void('0');" onclick="getNonConfFor('$RRNF');"></a> 
  ...
  <td class="text">$PRLOTN</td>
  <td class="text num">
SEGDTA;
 echo number_format($PDUORG,0,"","."); 
		echo <<<SEGDTA
</td>  
</tr>
<tr id="row-expand-for-$RRNF" style="display:none;">
	<td id="cell-expand-for-$RRNF" colspan="11"></td>
</tr>
...
<span id="list-paging-bottom" class="list-paging">
  <a id="previous-link-bottom" class="list-paging previous hidden" href="#" onclick="$.blockUI();">
    <span class="glyphicon glyphicon-triangle-left" style="text-decoration: none; font-size: 0.8em"></span> <span>Previous</span>
  </a>
  <a id="next-link-bottom" class="list-paging next hidden" href="#" onclick="$.blockUI();">
    <span>Next $listSize</span> <span class="glyphicon glyphicon-triangle-right" style="text-decoration: none; font-size: 0.8em"></span>
  </a>
</span>
</div>
...
SEGDTA;
	...
	function __construct()
	{
		...
		// Last Generated CRC: 262F558D 079075AB 84372997 43270E5C
		// Last Generated Date: 2024-06-25 10:14:57
		// Path: nonconf01.phw
	}
}
