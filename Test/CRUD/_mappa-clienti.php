...
class mappa_clienti extends WebSmartObject
{
	...
	protected $repArrivo = "";
	protected $googleApiKey = "AIzaSyCJnf3PhzjSRt3G76DF62K3CnsAXR-uoS8"; //AIzaSyCJnf3PhzjSRt3G76DF62K3CnsAXR-uoS8
	...
	protected function getPointsInBound() {
		...
		/*$ne_lat = str_replace(".",",",$ne_lat);
		$ne_lng = str_replace(".",",",$ne_lng);
		$sw_lat = str_replace(".",",",$sw_lat);
		$sw_lng = str_replace(".",",",$sw_lng);*/
		...
		while($row = $stmt->fetch(PDO::FETCH_ASSOC)) { 
			...
			$lng = $decCliente["CCLONG"];
	  		...
	  		$ABALPH = $decCliente["ABALPH"]; 
	  		$ALCTY1 = $decCliente["ALCTY1"]; 
	  		$ALCOUN = $decCliente["ALCOUN"]; 
	  		$ALADDZ = $decCliente["ALADDZ"]; 
	  		$ALADD2 = $decCliente["ALADD2"]; 
	  		$ALADDS = $decCliente["ALADDS"]; 
	  		
	  		$tmpArray = array(); 
	  		$tmpArray["status"] = "OK";  
	  		$tmpArray["ABAN8"] = mb_convert_encoding(trim($ABAN8), 'UTF-8', 'ISO-8859-1');  
	  		$tmpArray["ABALPH"] = mb_convert_encoding(trim($ABALPH), 'UTF-8', 'ISO-8859-1');  
	  		$tmpArray["ALCTY1"] = mb_convert_encoding(trim($ALCTY1), 'UTF-8', 'ISO-8859-1');  
	  		$tmpArray["ALCOUN"] = mb_convert_encoding(trim($ALCOUN), 'UTF-8', 'ISO-8859-1');  
	  		$tmpArray["ALADDZ"] = mb_convert_encoding(trim($ALADDZ), 'UTF-8', 'ISO-8859-1');  
	  		$tmpArray["ALADD2"] = mb_convert_encoding(trim($ALADD2), 'UTF-8', 'ISO-8859-1');
	  		$tmpArray["ALADDS"] = mb_convert_encoding(trim($ALADDS), 'UTF-8', 'ISO-8859-1');  
	  		$tmpArray["GLAT"] = $lat;  
	  		$tmpArray["GLNG"] = $lng;  
	  		...
	  		$tmpArray["fatt1"] = $fatt1; 
	  		$tmpArray["fatt2"] = $fatt2; 
	  		$tmpArray["fatt3"] = $fatt3; 
	...
	protected function getCoords() {
		...
		{ 
			...
			if($geoloc['status']=='OK') { 
			  $CCLATI = $geoloc['results'][0]['geometry']['location']['lat'];
			  $CCLONG = $geoloc['results'][0]['geometry']['location']['lng'];
			  $CCSTAT = 'OK';
			} else {
			  $CCLATI = 0;
			  $CCLONG = 0;
...
';
	        ...
	        var image = '/crud/websmart/maps/img/google-maps-marker-yellow.png';
	      	var ne = 0;
	      	var sw = 0;
	      	markers = new Array();
 	        ...
 	        for(i=0;i<boxes.length;i++) {
	        	...
	        	url = "?task=getPointsInBound&ne_lat="+escape(ne_lat)+"&ne_lng="+escape(ne_lng)+"&sw_lat="+escape(sw_lat)+"&sw_lng="+escape(sw_lng);
	        	$.getJSON(url,function(rtnpts) {
	        		...
	        		if(rtnpts[0].status!="ERR" && rtnpts[0].status!="NONE") {
	        			...
	        			for(j=0;j<rtnpts.length;j++) {
							...
							inBoundString = '<tr><td valign="top"><input class="btn btn-sm btn-success addcliente" type="button" onclick="addGiro(\''+rtnpts[j].ABAN8+'\')" value="Aggiungi" /></td><td><b>'+rtnpts[j].ABAN8+' - '+rtnpts[j].ABALPH+'</b><br><span class="indClienteMarker">'+rtnpts[j].ALADD2+',<br>'+rtnpts[j].ALCTY1+' ('+rtnpts[j].ALADDS+')</span></td><td nowrap>Fatturato '+y+': '+rtnpts[j].fatt1+'<br>Fatturato '+y1+': '+rtnpts[j].fatt2+'<br>Fatturato '+y2+': '+rtnpts[j].fatt3+'</td></tr>';	
...
,
	...
	function addGiro(ABAN8) { 
		...
		$.get(url,function(data){ 
			document.location.href = "?task=calcGiroV&GIUSER=$GIUSER&GITIPA=$GITIPA&GITIAR=$GITIAR&GINOPE=$GINOPE&GITIPO=$GITIPO&GIPAUS=$GIPAUS&GIMECL=$GIMECL&GIORDI=$GIORDI&GIPACL=$GIPACL&GIARCL=$GIARCL";
		});	
	} 
	...
	function getCoords() {
		...
		$.getJSON(url,function(data){
			for(j = 0;j < data.length; j++) {
				h = "<div";
				if(data[j].AD!="OK") h = h + " style='color:red;'";
				h = h + ">an8:"+data[j].AN+", stat:"+data[j].AD;
...
SEGDTA;
	...
	function __construct()
	{
		...
		// Last Generated CRC: CC7A4599 D8B8CDA3 4A2DF491 48143B5D
		// Last Generated Date: 2024-06-26 15:37:16
