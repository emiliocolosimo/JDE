...
class mappa_clienti extends WebSmartObject
{
	...
	public function runMain()
	{
		...
		switch ($this->pf_task)
		{
			...
			case 'getPointsInRouteBound':
			$this->getPointsInRouteBound();
			break;
			
			case 'savPointsInRouteBound':
			$this->savPointsInRouteBound();
			break;			
			
			case 'getPointsInMapBound':
			$this->getPointsInMapBound();
			break;
			
			case 'savPointsInMapBound':
			$this->savPointsInMapBound();
			break;	
	...
	protected function getPointsInRouteBound() {
	...
	protected function getPointsInMapBound() {
	...
	protected function savPointsInMapBound() {
		...
		while($row = $stmt->fetch(PDO::FETCH_ASSOC)) { 
			...
			$query = "INSERT INTO BCD_DATIV2.RGPGPM00F 
			(GPPROG,GPAN8,GPALPH,GPCTY1,GPCOUN,GPADDZ,GPADD2,GPADDS,GPLAT,GPLNG,GPFAT1,GPFAT2,GPFAT3,GPBAT1,GPTS) VALUES 
			(:GPPROG,:GPAN8,:GPALPH,:GPCTY1,:GPCOUN,:GPADDZ,:GPADD2,:GPADDS,:GPLAT,:GPLNG,:GPFAT1,:GPFAT2,:GPFAT3,:GPBAT1,:GPTS) WITH NC
			";
			...
			$stmt_ins->bindValue(':GPTS', $GPTS, PDO::PARAM_STR);
	...
	protected function savPointsInRouteBound() {
	...
	protected function calcGiroV() {
		...
		$query = "DELETE FROM BCD_DATIV2.RGPGPM00F WHERE GPPROG = ".$GTPROG." WITH NC ";	
		$res = $this->db_connection->exec($query);
...
<div class="col-sm-4" id="giroContainer"> 
	...
	<form name="formGiroComposer" id="formGiroComposer" class="smart-form" method="post" action="mappa-clienti-2.php">
	        ...
	        var image = '/crud/websmart/maps/img/google-maps-marker-red.png';
	      	...
	      	if("$ricalcInBound"!="N") {
	 	        ...
	 	        for(i=0;i<boxes.length;i++) {
		        	...
		        	url = "?task=savPointsInRouteBound&ne_lat="+escape(ne_lat)+"&ne_lng="+escape(ne_lng)+"&sw_lat="+escape(sw_lat)+"&sw_lng="+escape(sw_lng);
		        	$.getJSON(url,function(rtnpts) { 
		        		if(isLast) {
		        			getPointsInRouteBound();
...
SEGDTA;
 echo $this->repArrivo; 
		...
		function getPointsInRouteBound() {
			
	        var image = '/crud/websmart/maps/img/google-maps-marker-red.png';

        	url = "?task=getPointsInRouteBound&ts="+Date.now();
        	$.getJSON(url,function(rtnpts) {         		
	      	...
	      	markers = new Array();
		      	...
		      	if("$ricalcInBound"!="N") {
		 	        ...
		 	        for(i=0;i<boxes.length;i++) {
			        	...
			        	url = "?task=savPointsInRouteBound&GIMIFA=$GIMIFA&ne_lat="+escape(ne_lat)+"&ne_lng="+escape(ne_lng)+"&sw_lat="+escape(sw_lat)+"&sw_lng="+escape(sw_lng);
			        	$.getJSON(url,function(rtnpts) { 
			        		if(isLast) {
			        			getPointsInRouteBound();
			        			$("#inBoundPanelDiv").unblock();
		...
		echo <<<SEGDTA
			    ...
			    */  
				...
				google.maps.event.addListener(map, "bounds_changed", function() { 
					...
					jGPTS = Date.now();
		        	url = "?task=savPointsInMapBound&GIMIFA=$GIMIFA&ne_lat="+escape(ne_lat)+"&ne_lng="+escape(ne_lng)+"&sw_lat="+escape(sw_lat)+"&sw_lng="+escape(sw_lng)+"&GPTS="+jGPTS;
		        	$.getJSON(url,function(rtnpts) { 
		        		  console.log("savPointsInMapBound end");
						  getPointsInMapBound(jGPTS); 
		        	});					
...
SEGDTA;
 echo $this->repArrivo; 
		...
		function getPointsInMapBound(jGPTS) {
	        var image = '/crud/websmart/maps/img/marker-green.png';

        	url = "?task=getPointsInMapBound&GIVITU=$GIVITU&GIMIFA=$GIMIFA&GIRARI=$GIRARI&GITPCL_C=$GITPCL_C&GITPCL_CS=$GITPCL_CS&GITPCL_P=$GITPCL_P&GITPCL_PS=$GITPCL_PS&GITPCL_V=$GITPCL_V&GPTS="+jGPTS;
        	$.getJSON(url,function(rtnpts) {         		
				...
				console.log("getPointsInMapBound end");
		...
		function getPointsInRouteBound() {
        	...
        	url = "?task=getPointsInRouteBound&GIVITU=$GIVITU&GIMIFA=$GIMIFA&GIRARI=$GIRARI&GITPCL_C=$GITPCL_C&GITPCL_CS=$GITPCL_CS&GITPCL_P=$GITPCL_P&GITPCL_PS=$GITPCL_PS&GITPCL_V=$GITPCL_V&ts="+Date.now();
        	$.getJSON(url,function(rtnpts) {         		
...
SEGDTA;
	...
	function __construct()
	{
		...
		$this->pf_scriptname = 'mappa-clienti-2.php';
		$this->pf_wcm_set = 'PRODUZIONE';
		
		
		$this->xl_set_env($this->pf_wcm_set);
		
		// Last Generated CRC: A845C1FB 32525192 25811E3B EDE45B85
		// Last Generated Date: 2024-07-17 16:25:34
