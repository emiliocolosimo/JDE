...
class BankInfoInserter {
	...
	public function checkMandatoryFields($fieldsArray) {
		...
		$fieldsArray["AYAN8"] = $headerFields["customer_code"]; 
		if($headerFields["customer_type"]=="C") $fieldsArray["AYAN8"] = $fieldsArray["AYAN8"] + 1000000;
		 
		$fieldsArray["AYUSER"] = $this->getUtenteJDE($headerFields["crm_user"]);
		$fieldsArray["AYPID"] = $fieldsArray["AYUSER"];
		$fieldsArray["AYTORG"] = $fieldsArray["AYUSER"];			
	...
	public function execInsert() {
		...
		//ricavo progressivo:
		$query = "SELECT MAX(AYUKID) + 1 AS NXTPRG 
		FROM ".$this->curLib.".F0030 ";
		$res = odbc_exec($this->odbc_conn,$query);
		if(!$res) {
			$hasErrors = true;
			$arrErrors[$xe]["field"] = "";
			$arrErrors[$xe]["msg"] = "F0030:".odbc_errormsg($this->odbc_conn);
			$xe++;			
			return array("hasErrors"=>$hasErrors,"arrErrors"=>$arrErrors);
		}
		$row = odbc_fetch_array($res);
		if(!$row || !isset($row["NXTPRG"])) {
			$hasErrors = true;
			$arrErrors[$xe]["field"] = "";
			$arrErrors[$xe]["msg"] = "F0030:".odbc_errormsg($this->odbc_conn);
			$xe++;			
		...
		$fieldsArray["AYUKID"] = $row["NXTPRG"];
		//ricavo progressivo [f]		
		
		$query = "INSERT INTO ".$this->curLib.".F0030 (".implode(",",$inputFields).") VALUES("; 
		for($i=0;$i<count($inputFields);$i++) { 
			if($i>0) $query.= ", ";
