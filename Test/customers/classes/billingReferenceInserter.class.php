...
class BillingReferenceInserter {
	...
	public function checkMandatoryFields($fieldsArray) {
		...
		$fieldsArray["AGAN8"] = $headerFields["customer_code"];
		if($headerFields["customer_type"]=="C") $fieldsArray["AGAN8"] = $fieldsArray["AGAN8"] + 1000000; 
		 
		$fieldsArray["AGUSER"] = $this->getUtenteJDE($headerFields["crm_user"]);
		$fieldsArray["AGPID"] = $fieldsArray["AGUSER"];
		$fieldsArray["AGTORG"] = $fieldsArray["AGUSER"];	
	...
	public function execInsert() {
		...
		$query = "INSERT INTO ".$this->curLib.".F01017 (".implode(",",$inputFields).") VALUES("; 
		for($i=0;$i<count($inputFields);$i++) { 
			if($i>0) $query.= ",";
