...
class ContactUpdater {
	...
	public function deleteAllRows() { 
		...
		$query  = "DELETE FROM ".$this->curLib.".F0111 WHERE WWAN8 = ? WITH NC";
 		$pstmt = odbc_prepare($this->odbc_conn,$query);
		if(!$pstmt) {
			$hasErrors = true;
			$arrErrors[0]["field"] = "";
			$arrErrors[0]["msg"] = "F0111:".odbc_errormsg($this->odbc_conn);		
			return array("hasErrors"=>$hasErrors,"arrErrors"=>$arrErrors);
		}		
		$arrParams = array();	
		$arrParams[] = $headerFields["customer_code"];
		$res = odbc_execute($pstmt,$arrParams);
		if(!$res) {
			$hasErrors = true;
			$arrErrors[0]["field"] = "";
	...
	public function checkMandatoryFields($fieldsArray) {
		...
		$fieldsArray["ABDC"] = substr(str_replace(array(" "),array(""),$headerFields["customer_name"]),0,40);
	...
	public function recordExists() {
		...
		$query = "SELECT 'S' AS RECEX FROM ".$this->curLib.".F0111 WHERE 
		";
	...
	public function execUpdate() {
		...
		$query = "UPDATE ".$this->curLib.".F0111 SET "; 
		$xi = 0;
		for($i=0;$i<count($inputFields);$i++) { 
