...
class PhoneInserter {
	...
	public function checkMandatoryFields($fieldsArray) {
		...
		$fieldsArray["WPAN8"] = $headerFields["customer_code"];
		if($headerFields["customer_type"]=="C") $fieldsArray["WPAN8"] = $fieldsArray["WPAN8"] + 1000000;
		 
		$fieldsArray["WPUSER"] = $this->getUtenteJDE($headerFields["crm_user"]);
		$fieldsArray["WPPID"] = $fieldsArray["WPUSER"];
		$fieldsArray["WPTORG"] = $fieldsArray["WPUSER"];	
	...
	public function execInsert() {
		...
		$query = "INSERT INTO ".$this->curLib.".F0115 (".implode(",",$inputFields).") VALUES("; 
		for($i=0;$i<count($inputFields);$i++) { 
			if($i>0) $query.= ",";
		...
		if(!$pstmt) {
			$hasErrors = true;
			$arrErrors[$xe]["field"] = "";
			$arrErrors[$xe]["msg"] = "F0115:".odbc_errormsg($this->odbc_conn);
			$xe++;			
			return array("hasErrors"=>$hasErrors,"arrErrors"=>$arrErrors);
		}
		$arrParams = array();	
