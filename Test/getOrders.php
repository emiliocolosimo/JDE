...
if($resArray) {
	if(isset($resArray['filters'])) {
		...
		for($i=0;$i<count($arrFilters);$i++) {
			...
			if($field=="STATO_ORDINE") $field = " (CASE WHEN PDUREC = 0 THEN 'ORDINI APERTI' ELSE CASE WHEN PXOPRC = 'TRAN' THEN 'MERCE IN TRANSITO' ELSE 'MERCE RICEVUTA' END END) ";
			...
			if($type=="le") $whrClause .= " ".$filterMode." (".$field." <= '".$value."') ";
			if($type=="ge") $whrClause .= " ".$filterMode." (".$field." >= '".$value."') ";
...
FROM JRGDTA94C.F4311JB 
LEFT JOIN JRGDTA94C.F0111 ON PDAN8 = WWAN8 AND WWIDLN = 0 
