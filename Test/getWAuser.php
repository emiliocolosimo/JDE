...
$data = json_decode($postedBody, true);
$userTest = $data['user'];
$pwdTest = $data['pwd'];
...
$query = "SELECT 
TRIM(DRKY) AS CODE,
TRIM(DRDL01) AS CUSTOMER, 
TRIM(DRDL02) AS PASSWORD,
TRIM(DRSPHD) AS ADMIN 
FROM JRGCOM94T.F0005
WHERE DRSY='55' AND DRRT='WA'
"; 
...
while($row = odbc_fetch_array($result)){
		...
		// Converte il campo CUSTOMER e PASSWORD in minuscolo
		$row['CUSTOMER'] = strtolower($row['CUSTOMER']);
		$row['PASSWORD'] = strtolower($row['PASSWORD']);
