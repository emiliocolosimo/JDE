...
$SHDOCO = '';
if(isset($_REQUEST["SHDOCO"])) $SHDOCO = trim($_REQUEST["SHDOCO"]);
$SHDCTO = '';
if(isset($_REQUEST["SHDCTO"])) $SHDCTO = trim($_REQUEST["SHDCTO"]); 
$SHKCOO = '';
if(isset($_REQUEST["SHKCOO"])) $SHKCOO = trim($_REQUEST["SHKCOO"]); 

if($SHDOCO=='') die('{"stat":"err","msg":"Parametro Order Number obbligatorio"}');
if($SHDCTO=='') die('{"stat":"err","msg":"Parametro Order Type obbligatorio"}');
if($SHKCOO=='') die('{"stat":"err","msg":"Parametro Order Company obbligatorio"}');
...
$SHAN8 = '';
$SHCACT = '';
$query = "SELECT trim(SHRYIN) as SHRYIN, 
trim(SHAN8) as SHAN8, 
trim(SHCACT) as SHCACT, 
trim(SHPTC) as SHPTC   
FROM JRGDTA94C.F4201 
WHERE SHDOCO = ? 
AND SHDCTO = ? 
AND SHKCOO = ? 
";
...
if($pstmt) {
	$arrParams = array();	
	$arrParams[] = trim($SHDOCO);
	$arrParams[] = trim($SHDCTO);
	$arrParams[] = trim($SHKCOO);
	 
	$res = odbc_execute($pstmt,$arrParams);
	if($res) {
		$row = odbc_fetch_array($pstmt);
		if(isset($row["SHRYIN"])) $SHRYIN = $row["SHRYIN"];
		if(isset($row["SHAN8"])) $SHAN8 = $row["SHAN8"];
		if(isset($row["SHCACT"])) $SHCACT = $row["SHCACT"];
		if(isset($row["SHPTC"])) $SHPTC = $row["SHPTC"];
		
		
	}
}

if($SHRYIN=='' || $SHAN8=='') die('{"stat":"err","msg":"Ordine non trovato"}');
