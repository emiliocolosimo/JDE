...
if($pstmt) {
	...
	if($res) {
		while($row = odbc_fetch_array($pstmt)) {
			...
			$dspVal = true;
			if($filter_desc!="") {
				if(strpos(strtolower($PNPTD), strtolower($filter_desc)) === false) $dspVal = false;
			}			
			
			if($dspVal) {
				if($r>0) echo ',';
				echo '{"payment_terms_code":'.json_encode($PNPTC).',"payment_terms_descr":'.json_encode($PNPTD).'}';
				$r++;
			}
		} 
