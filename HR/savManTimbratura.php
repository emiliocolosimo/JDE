...
$dateObj = DateTime::createFromFormat('Y-m-d', $transitRecord["FullDate"]);
$transitRecord["Day"] = $dateObj->format('d');
$transitRecord["Month"] = $dateObj->format('m');
$transitRecord["Year"] = $dateObj->format('y');
$transitRecord["WDay"] = $dateObj->format('w');
    ...
    <form method="POST" class="row g-3">
        ...
        <div class="col-md-4">
            ...
            <select name="ReasonCode" class="form-select" required>
                <option value="Ingresso" selected>Ingresso</option>
                <option value="Uscita" selected>Uscita</option>
                <option value="Pausa" selected>Pausa</option>
            </select>
        </div>
...
<div class="col-md-6">
    ...
    <input type="date" name="FullDate" class="form-control" value="<?= $now->format('Y-m-d') ?>" required>
</div>
...
function saveTransitRecord($conn, $requestType, $codDipendente, $tr) {
    $STTPRE = $requestType;
    $STTYPE = "4";
switch (strtoupper(trim($tr["ReasonCode"]))) {
    case 'INGRESSO':
        $STSENS = 'E';
        $STRECO = '0000';
        break;
    ...
    $STWDAY = $tr["WDay"];
    $STBADG = $tr["Badge"];
    $STCDDI = trim(string: $codDipendente);
    $STHOUR = $tr["Hour"];
    $STMINU = $tr["Minute"];
    $STSECO = "00";
    $STDAY  = $tr["Day"];
