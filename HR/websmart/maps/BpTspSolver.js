...
(function() {
  ...
  var avoidTolls = false; // Whether to avoid highways. False by default.
  var travelMode;
  var distIndex;
  ...
  function nextChunk(mode) {
	...
	chunkNode--;
      ...
      var myGebDirections = new google.maps.DirectionsService();
		//QUIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIIII
		var milliseconds = new Date().getTime();
		var curmillis = 0;

		while(curmillis<1000) {
			curmillis = new Date().getTime();
			curmillis = curmillis - milliseconds;
		}    
  ...
  BpTspSolver.prototype.setAvoidTolls = function(avoidt) {
    avoidTolls = avoidt;
  }  
  ...
  BpTspSolver.prototype.getRouteLatLngs = function() {
	 
	routeLatLngs = new Array(); 
	for(i=0;i<gebDirectionsResult.routes.length;i++) {
		lg = gebDirectionsResult.routes[i].legs; 
		for(j=0;j<lg.length;j++) {
			st = lg[j].steps;  
			for(x=0;x<st.length;x++) {
				ln = st[x].lat_lngs;  
				for(f=0;f<ln.length;f++) {
					routeLatLngs.push(ln[f]); 	
				}	
			}
		}
	}
