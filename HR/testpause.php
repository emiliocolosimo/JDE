 ...
 //     $AUTH = $BDAUTH;
   //   $PASS = $BDPASS;
      ...
      $Trasmessa = $STTRAS;
        ...
        // Calcolo durata totale lavoro e pausa per ciascun dipendente
        $statisticheDipendenti = [];
        foreach ($presentiInSede as $chiave => $dati) {
            $orarilavoro = array_filter($dati['orarilavoro'] ?? []);
            $oraripause = array_filter($dati['oraripause'] ?? []);
            $totLavoro = 0;
            $totPausa = 0;
            ...
            for ($i = 0; $i < count($orarilavoro); $i += 2) {
                if (!empty($orarilavoro[$i]) && !empty($orarilavoro[$i + 1])) {
                    try {
                        $start = new DateTime($orarilavoro[$i]);
                        $end = new DateTime($orarilavoro[$i + 1]);
                        $totLavoro += abs($end->getTimestamp() - $start->getTimestamp());
                    } catch (Exception $e) {}
                }
            }
            ...
            for ($i = 0; $i < count($oraripause); $i += 2) {
                if (!empty($oraripause[$i]) && !empty($oraripause[$i + 1])) {
                    try {
                        $start = new DateTime($oraripause[$i]);
                        $end = new DateTime($oraripause[$i + 1]);
                        $totPausa += abs($end->getTimestamp() - $start->getTimestamp());
                    } catch (Exception $e) {}
                }
            }

            $statisticheDipendenti[$chiave] = [
                'cognome' => $dati['cognome'] ?? '',
                'nome' => $dati['nome'] ?? '',
                'ore_lavorate' => sprintf('%dh %02dm', floor($totLavoro / 3600), ($totLavoro / 60) % 60),
                'minuti_pausa' => floor($totPausa / 60)
            ];
        }
        ...
        if ($_SESSION["filtroTipo"] === 'inpausa') {
            ...
            if (isset($_SESSION["chkMancanti"]) && $_SESSION["chkMancanti"]) {
                foreach ($presentiInSede as $chiave => $dati) {
                    ...
                    if (!empty($orarilavoro[0]) && (!isset($orarilavoro[1]) || empty($orarilavoro[1]))) {
                        $now = new DateTime();
                    ...
                    if (!empty($orarilavoro[2]) && (!isset($orarilavoro[3]) || empty($orarilavoro[3]))) {
                        $now = new DateTime();
                    ...
                    /*
                    // NUOVA LOGICA: conta timbrature tra le 08:00-12:29 e 13:30-17:00
                    $contaMattina = 0;
                    $contaPomeriggio = 0;
                    foreach ($orarilavoro as $ora) {
                        if (empty($ora)) continue;
                        try {
                            $orario = new DateTime($ora);
                            $hhmm = (int) $orario->format('Hi');
                            if ($hhmm >= 800 && $hhmm <= 1229) $contaMattina++;
                            if ($hhmm >= 1330 && $hhmm <= 1700) $contaPomeriggio++;
                        } catch (Exception $e) {}
                    }

                    if ($contaMattina % 2 !== 0 || $contaPomeriggio % 2 !== 0) {
                        $mancante = true;
                    }
                    */
        ...
        // --- AGGIUNTA: riepilogo statistiche dipendenti ---
        echo "<h4 class='mt-3'>Statistiche Dipendenti (giorno selezionato)</h4><ul>";
        foreach ($statisticheDipendenti as $stat) {
            echo "<li>{$stat['cognome']} {$stat['nome']}: {$stat['ore_lavorate']} lavoro, {$stat['minuti_pausa']} minuti pausa</li>";
        }
        echo "</ul>";
...
SEGDATA;
	...
	protected function printPageFooter()
	{
		...
		<!-- Chatbox AI -->
		<div id="chatbox" style="position: fixed; bottom: 20px; right: 20px; width: 300px; background: #f8f9fa; border: 1px solid #ccc; border-radius: 8px; padding: 10px; box-shadow: 0 0 10px rgba(0,0,0,0.2); z-index: 9999;">
		  <div style="font-weight: bold; margin-bottom: 5px;">Chat AI - Info Dipendente</div>
		  <textarea id="chatInput" rows="2" placeholder="Scrivi il nome o badge..." style="width: 100%; resize: none;"></textarea>
		  <button onclick="inviaChat()" class="btn btn-primary btn-sm mt-2">Chiedi</button>
		  <div id="chatRisposta" style="margin-top: 10px; font-size: 0.9rem;"></div>
		</div>
		</body>
		<script>
		async function inviaChat() {
		  const domanda = document.getElementById('chatInput').value;
		  const rispostaBox = document.getElementById('chatRisposta');
		  rispostaBox.innerHTML = "Attendere...";

		  try {
			const response = await fetch('/AI/connect.php', {
			  method: 'POST',
			  headers: { 'Content-Type': 'application/json' },
			  body: JSON.stringify({ message: "Dammi info sul dipendente: " + domanda })
			});

			const data = await response.json();
			rispostaBox.innerHTML = data.reply || "Nessuna risposta ricevuta.";
		  } catch (err) {
			rispostaBox.innerHTML = "Errore nella richiesta.";
		  }
		}
		</script>
		</html>
