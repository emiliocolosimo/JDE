...
ini_set("error_log", "/www/php80/htdocs/logs/getNewOrder/php-error.log");
...
$query = "SELECT 
    ...
    U.SDAN8, 
    U.SDASN, 
    TRIM(U.SDDOCO) AS SDDOCO,
    U.SDDOC,
    U.SDDCT,
    U.SDDELN,
    U.SDCARS,
    (DECIMAL(SDLNID / 100, 8, 0)) AS SDLNID,
    TRIM(COALESCE(WWMLNM, '')) AS WWMLNM,
    SUM(DECIMAL(U.SDSOQS / 100, 14, 4)) AS SDSOQS, 
    SUM(DECIMAL(U.SDAEXP / 100, 14, 4)) AS SDAEXP,  
    SUM(DECIMAL(U.SDFEA / 100, 14, 4)) AS SDFEA
FROM (
    SELECT 
CAST(RRN(".$curLib.".F4211) AS INTEGER) AS RRN,
        'F4211' AS FILE,
        CASE 
        ...
        trim(coalesce((select min(WWMLNM) from ".$curLib.".F0111 where ".$curLib.".F4211.SDCARS=".$curLib.".F0111.WWAN8 and ".$curLib.".F0111.WWIDLN=0), '')) as SDCARS,
    ...
    FROM ".$curLib.".F4211 
      ...
      AND SDLTTR NOT IN ('980', '988')

    UNION ALL

    SELECT 
  CAST(RRN(".$curLib.".F42119) AS INTEGER) AS RRN ,
     'F42119' AS FILE,    
    CASE 
            WHEN SDDCTO IN ('SQ', 'OF') THEN 'Offerta'
            WHEN SDDCTO IN ('OB') THEN 'Richiamo'
        ...
        TRIM(SDDCT) AS SDDCT,
        TRIM(SDDELN) AS SDDELN,
        trim(coalesce((select min(WWMLNM) from ".$curLib.".F0111 where ".$curLib.".F42119.SDCARS=".$curLib.".F0111.WWAN8 and ".$curLib.".F0111.WWIDLN=0), '')) as SDCARS,
        TRIM(SDLNID) AS SDLNID,
        TRIM(SDSOQS) AS SDSOQS,
        TRIM(SDAEXP) AS SDAEXP,
        TRIM(SDFEA) AS SDFEA
...
LEFT JOIN ".$curLib.".F0111 
    ON U.SDAN8 = WWAN8 AND WWIDLN = 0 

    ";
if ($whrClause != "") $query .= $whrClause;
    $query .= "

    GROUP BY U.RRN , U.FILE , U.TIPO, U.UFFICIO, U.SDLITM  ,  U.SDFRGD, U.SDEUSE, U.SDUPRC, U.SDFUP, U.SDUOM4, U.SDCRCD, U.SDPDDJ, U.SDADDJ, U.SDAN8, U.SDASN, U.SDDOCO, U.SDDOC , SDDCT, SDDELN, SDCARS, DECIMAL(SDLNID / 100, 8, 0) , WWMLNM 
    "; 
