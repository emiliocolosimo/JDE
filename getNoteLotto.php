...
ini_set("error_log", "/www/php80/htdocs/logs/getNoteLotto/php-error.log");
...
TRIM(IOLITM) AS IOLITM, 
trim(IOLOTN) AS IOLOTN, 

TRIM(
	COALESCE(
	  (
  SELECT LISTAGG (trim(CYWTXT), ', ') WITHIN GROUP (ORDER BY CYlin)   
  FROM JRGDTA94C.f00164, JRGDTA94C.f0016 WHERE CYSERK = C6SERK AND    
  C6WAPP = '*P4108' AND C6CKY2 = IOMCu || (DIGITS(ioITM)) || ioLOTN                   
   ),
	  ''
	)
  ) AS NOTA 
...
JRGDTA94C.f0016 , 
JRGDTA94C.F4108 
WHERE CYSERK=C6SERK and 
c6wapp='*P4108' AND
c6cky2=IOMCU||(DIGITS(ioitm))||IOLOTN
"; 
